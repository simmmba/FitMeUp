                    ...
                    if(m) {
                        return (
                            <div className="creditHistoryBox" key={m.id}>
                                <div className={m.type==="charge"?("line center type blue"):
                                    (m.type==="income"?("line center type blue"):("line center type red"))}>{state[m.type]}</div>
                                ...
                                <div className={m.type==="charge"?("line center amount blue"):
                                    (m.type==="income"?("line center amount blue"):("line center amount red"))}>
                                    {m.amount} KRW</div>
                                <div className="line center date">{m.createdAt.substring(0, 10)} {m.createdAt.substring(11, 16)}</div>
                            </div>
                        )
                    }
                })}
            </div>
            <Pagination className="topMargin center"
                        size={"small"} page={page}
                        color={"secondary"} count={count}
